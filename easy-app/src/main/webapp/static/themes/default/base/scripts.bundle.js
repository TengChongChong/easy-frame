...
var mUtil = function () {
     ...
     * 设备宽度
     *
     * @type {{xl: number, md: number, sm: number, lg: number}}
     */
    var breakpoints = {
        sm: 544, // 小屏 / 手机
        md: 768, // 中等屏幕 / 平板电脑
    ...
    return {
        ...
        hasFixedPositionedParent: function (el) {
            while (el && el !== document) {
                var position = mUtil.css(el, 'position');
...
var mApp = function () {
    ...
    var initDropzone = function (options) {
        ...
        if (mUtil.isFunction(options.success)) {
        ...
        if (mUtil.isFunction(options.complete)) {
    ...
    var closeTabByUrl = function (url) {
        if (top.location !== self.location && typeof siteTabs === 'undefined') {
            // 有父页面,并且 siteTabs is null
    ...
    var initDatePicker = function (selector) {
        ...
        $(selector).each(function () {
            ...
            if (mUtil.isBlank(config.pickerPosition)) {
                config.pickerPosition = 'bottom-right';
            }
            if (mUtil.isBlank(config.format)) {
    ...
    var Plugin = {
        ...
        build: function () {
            ...
            defaultOptions.toLeft.click(function () {
                ...
                if (marginLeft !== 0) {
                    var tabsScrollWidth = Plugin.calcWidth(defaultOptions.contabsScroll); // tab 工具条可视宽度
...
var mTool = function () {
    ...
    var importData = function (importCode) {
        mApp.openPage('导入数据', basePath + '/auth/sys/import/excel/data/' + importCode);
    };
    /**
     * 保存数据
     *
    ...
    var checkBaseUrl = function () {
        if (mUtil.isNotBlank(baseUrl)) {
            ...
            printWarn('请使用mTool.setBaseUrl(\'string\');设置业务url根目录');
            return false;
        }
    };
    ...
    $.fn[pluginName] = function (options) {
        ...
        var Plugin = {
            ...
            extractTable: function () {
                var columns = [];
                var headers = $(datatable).find('tr:first-child th').get().map(function (cell) {
            ...
            setupTemplateCell: function (tablePart) {
                ...
                $(tablePart).find('.' + pfx + 'datatable__row').each(function (tri, tr) {
                    ...
                    if (typeof obj === 'undefined') {
                        ...
                        $(tr).find('.' + pfx + 'datatable__cell').each(function (tdi, td) {
                            // 根据列名称获取列设置
                            var column = $.grep(columns, function (n) {
            ...
            dataRender: function (action) {
                ...
                var afterGetData = function (result) {
                    if (result == null || mTool.httpCode.success === result.code) {
            ...
            getData: function () {
                ...
                var ajaxParams = {
                    ...
                    method: 'GET',
                    data: {},
                    timeout: Plugin.getOption('data.source.read.timeout') || 30 * 1000 // 超时时间,默认30s
                };
                    ...
                    updateInfo: function () {
                        ...
                        $(pg.pager).find('.' + pfx + 'datatable__pager-info').find('.' + pfx + 'datatable__pager-detail').html(Plugin.dataPlaceholder(
                            Plugin.getOption('translate.toolbar.pagination.items.info'), {
                                ...
                                pg.meta.size >= pg.meta.total ? pg.meta.total : pg.meta.size, total: pg.meta.total
                            }));
                    },
